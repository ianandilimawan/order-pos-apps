...
class ProductTable extends PowerGridComponent
{
    ...
    {
        if (auth()->user() && (auth()->user()->hasPermission('delete-products') || auth()->user()->hasPermission('edit-products'))) {
            $this->showCheckBox();
        }
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('description')
            ->add('price')
            ->add('is_available_display', function (Product $row) {
                if ($row->is_available) {
            ...
            ->add('sort')
            ->add('show_display', function (Product $row) {
                if ($row->show) {
                    return '<span class="px-2 py-0.5 text-xs font-semibold rounded-full bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300">Active</span>';
                }
                return '<span class="px-2 py-0.5 text-xs font-semibold rounded-full bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300">Inactive</span>';
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Category Id', 'category_id')->sortable()->searchable(),
            ...
            Column::make('Is Available', 'is_available_display'),
    ...
    {
        return [
            Filter::boolean('is_available_display', 'is_available'),
