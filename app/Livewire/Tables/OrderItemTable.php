...
class OrderItemTable extends PowerGridComponent
{
    use WithExport;
    public string $tableName = 'orderItem-table';
    public string $sortField = 'id';
    public string $sortDirection = 'asc';

    public function setUp(): array
    {
        if (auth()->user() && auth()->user()->hasPermission('delete-order_items')) {
            $this->showCheckBox();
        }

        return [
            PowerGrid::exportable('export_order_items_' . now()->format('Ymd_His'))
    ...
    public function datasource(): Builder
    {
        ...
        if (Schema::hasColumn('order_items', 'sort')) {
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('subtotal')
            ->add('notes')
            ->add('action', function (OrderItem $row) {
                $actions = '<div class="flex items-center justify-center gap-1">';

                if (auth()->user() && auth()->user()->hasPermission('view-order_items')) {
                    $viewUrl = route('admin.order_items.show', $row->id);
                ...
                if (auth()->user() && auth()->user()->hasPermission('edit-order_items')) {
                    $editUrl = route('admin.order_items.edit', $row->id);
                ...
                if (auth()->user() && auth()->user()->hasPermission('delete-order_items')) {
                    $deleteUrl = route('admin.order_items.destroy', $row->id);
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Product Id', 'product_id')->sortable()->searchable(),
    ...
    public function bulkDeleteConfirmed($ids, $model): void
    {
        ...
        try {
            ...
            ActivityLogService::logBulkDelete(OrderItem::class, count($ids), $ids);
