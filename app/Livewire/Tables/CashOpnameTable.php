...
class CashOpnameTable extends PowerGridComponent
{
    use WithExport;
    public string $tableName = 'cashOpname-table';
    public string $sortField = 'id';
    public string $sortDirection = 'asc';

    public function setUp(): array
    {
        if (auth()->user() && auth()->user()->hasPermission('delete-cash_opnames')) {
            $this->showCheckBox();
        }

        return [
            PowerGrid::exportable('export_cash_opnames_' . now()->format('Ymd_His'))
    ...
    {
        $query = CashOpname::query();

        if (Schema::hasColumn('cash_opnames', 'sort')) {
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ->add('id')
            ->add('user_id')
            ->add('opname_date_formatted', fn ($model) => $model->opname_date ? \Carbon\Carbon::parse($model->opname_date)->format('d/m/Y') : '-')
            ->add('expected_cash')
            ->add('actual_cash')
            ->add('expected_qris')
            ->add('actual_qris')
            ->add('expected_transfer')
            ->add('actual_transfer')
            ->add('difference')
            ->add('status')
            ->add('notes')
            ->add('action', function (CashOpname $row) {
                $actions = '<div class="flex items-center justify-center gap-1">';

                if (auth()->user() && auth()->user()->hasPermission('view-cash_opnames')) {
                    $viewUrl = route('admin.cash_opnames.show', $row->id);
                ...
                if (auth()->user() && auth()->user()->hasPermission('edit-cash_opnames')) {
                    $editUrl = route('admin.cash_opnames.edit', $row->id);
                ...
                if (auth()->user() && auth()->user()->hasPermission('delete-cash_opnames')) {
                    $deleteUrl = route('admin.cash_opnames.destroy', $row->id);
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('User Id', 'user_id')->sortable()->searchable(),
            Column::make('Opname Date', 'opname_date_formatted', 'opname_date')->sortable()->searchable(),
            Column::make('Expected Cash', 'expected_cash')->sortable()->searchable(),
            Column::make('Actual Cash', 'actual_cash')->sortable()->searchable(),
            Column::make('Expected Qris', 'expected_qris')->sortable()->searchable(),
            Column::make('Actual Qris', 'actual_qris')->sortable()->searchable(),
            Column::make('Expected Transfer', 'expected_transfer')->sortable()->searchable(),
            Column::make('Actual Transfer', 'actual_transfer')->sortable()->searchable(),
            Column::make('Difference', 'difference')->sortable()->searchable(),
    ...
    public function bulkDeleteConfirmed($ids, $model): void
    {
        ...
        try {
            CashOpname::whereIn('id', $ids)->delete();
            ActivityLogService::logBulkDelete(CashOpname::class, count($ids), $ids);
