...
class PromoTable extends PowerGridComponent
{
    use WithExport;
    public string $tableName = 'promo-table';
    public string $sortField = 'id';
    public string $sortDirection = 'asc';

    public function setUp(): array
    {
        if (auth()->user() && auth()->user()->hasPermission('delete-promos')) {
            $this->showCheckBox();
        }

        return [
            PowerGrid::exportable('export_promos_' . now()->format('Ymd_His'))
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ->add('id')
            ->add('code')
            ->add('type_display', function(Promo $row) {
                return ucfirst($row->type);
            })
            ->add('value', function(Promo $row) {
                if ($row->type == 'percentage') {
                    return number_format($row->value, 0, ',', '.') . '%';
                }
                return 'Rp ' . number_format($row->value, 0, ',', '.');
            })
            ->add('is_active_display', function (Promo $row) {
                if ($row->is_active) {
            ...
            ->add('valid_until', function(Promo $row) {
                return $row->valid_until ? $row->valid_until->format('d/m/Y H:i') : 'No Limit';
            })
            ->add('action', function (Promo $row) {
                $actions = '<div class="flex items-center justify-center gap-1">';

                // View Details Button
                if (auth()->user() && auth()->user()->hasPermission('view-promos')) { 
                    $viewUrl = route('admin.promos.show', $row->id);
                ...
                if (auth()->user() && auth()->user()->hasPermission('edit-promos')) {
                    $editUrl = route('admin.promos.edit', $row->id);
                ...
                if (auth()->user() && auth()->user()->hasPermission('delete-promos')) {
                    $deleteUrl = route('admin.promos.destroy', $row->id);
    ...
    public function bulkDeleteConfirmed($ids, $model): void
    {
        try {
            ...
            ActivityLogService::logBulkDelete(Promo::class, count($ids), $ids);
            
            $this->js('window.pgBulkActions.clearAll()');
            $this->dispatch('notify', type: 'success', message: count($ids) . ' Promos have been deleted.');
        } catch (\Exception $e) {
            $this->dispatch('notify', type: 'error', message: 'Failed to delete Promos.');
        }
    }
}
