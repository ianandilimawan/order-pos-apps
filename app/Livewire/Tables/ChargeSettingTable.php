...
class ChargeSettingTable extends PowerGridComponent
{
    use WithExport;
    public string $tableName = 'chargeSetting-table';
    public string $sortField = 'id';
    public string $sortDirection = 'asc';

    public function setUp(): array
    {
        if (auth()->user() && auth()->user()->hasPermission('delete-charge_settings')) {
            $this->showCheckBox();
        }

        return [
            PowerGrid::exportable('export_charge_settings_' . now()->format('Ymd_His'))
    ...
    {
        $query = ChargeSetting::query();

        if (Schema::hasColumn('charge_settings', 'sort')) {
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('value')
            ->add('applies_to')
            ->add('is_active_display', function (ChargeSetting $row) {
                if ($row->is_active) {
            ...
            ->add('sort')
            ->add('action', function (ChargeSetting $row) {
                $actions = '<div class="flex items-center justify-center gap-1">';

                if (auth()->user() && auth()->user()->hasPermission('view-charge_settings')) {
                    $viewUrl = route('admin.charge_settings.show', $row->id);
                ...
                if (auth()->user() && auth()->user()->hasPermission('edit-charge_settings')) {
                    $editUrl = route('admin.charge_settings.edit', $row->id);
                ...
                if (auth()->user() && auth()->user()->hasPermission('delete-charge_settings')) {
                    $deleteUrl = route('admin.charge_settings.destroy', $row->id);
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Value', 'value')->sortable()->searchable(),
            Column::make('Applies To', 'applies_to')->sortable()->searchable(),
            Column::make('Is Active', 'is_active_display'),
    ...
    public function triggerBulkDelete(?array $ids = null): void
    {
        ...
        $this->dispatch('confirm-bulk-delete', [
            'ids' => $ids,
            'model' => 'App\\\\Models\\\\ChargeSetting',
    ...
    public function bulkDeleteConfirmed($ids, $model): void
    {
        ...
        try {
            ChargeSetting::whereIn('id', $ids)->delete();
            ActivityLogService::logBulkDelete(ChargeSetting::class, count($ids), $ids);
