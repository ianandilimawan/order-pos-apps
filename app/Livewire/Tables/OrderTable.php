...
class OrderTable extends PowerGridComponent
{
    ...
    {
        if (auth()->user() && auth()->user()->hasPermission('delete-order')) {
            $this->showCheckBox();
        }

        return [
            PowerGrid::exportable('export_orders_' . now()->format('Ymd_His'))
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('action', function (Order $row) {
                ...
                if (auth()->user() && auth()->user()->hasPermission('edit-order')) {
    ...
    public function bulkDeleteConfirmed($ids, $model): void
    {
        ...
        try {
            ...
            ActivityLogService::logBulkDelete(Order::class, count($ids), $ids);
