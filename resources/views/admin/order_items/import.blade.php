...
@section('title', 'Import Order Items')

@section('content')
    <div class="space-y-6">
        <!-- Page Header -->
        <div>
            <h1 class="lg:text-2xl text-xl font-semibold text-gray-900 dark:text-white">Import Order Items</h1>
            <p class="mt-1 lg:text-sm text-xs text-gray-600 dark:text-gray-400">Upload a CSV or Excel file (.csv, .xlsx, .xls) to import orderitems</p>
        </div>
...
@if (session('import_errors'))
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
            <form x-data="ajaxForm" @submit.prevent="submit" action="{{ route('admin.order_items.import') }}" method="POST" enctype="multipart/form-data" class="lg:p-8 px-4 py-4 space-y-6">
                @csrf
                ...
                <div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <div class="flex gap-2">
                            <a href="{{ route('admin.order_items.downloadSample', ['format' => 'csv']) }}"
                            ...
                            <a href="{{ route('admin.order_items.downloadSample', ['format' => 'xlsx']) }}"
                    ...
                    <div class="overflow-x-auto">
                        <pre class="text-xs text-gray-600 dark:text-gray-400 font-mono">order_id,product_id,quantity,price,subtotal,notes
123,123,123,123,123,Sample Value</pre>
                    </div>
                </div>

                <!-- Actions -->
                <div class="flex items-center justify-end gap-4 pt-8 border-t-2 border-gray-100 dark:border-gray-700 mt-8">
                    <a href="{{ route('admin.order_items.index') }}"
