...
@section('title', 'Import Dining Tables')

@section('content')
    <div class="space-y-6">
        <!-- Page Header -->
        <div>
            <h1 class="lg:text-2xl text-xl font-semibold text-gray-900 dark:text-white">Import Dining Tables</h1>
            <p class="mt-1 lg:text-sm text-xs text-gray-600 dark:text-gray-400">Upload a CSV or Excel file (.csv, .xlsx, .xls) to import diningtables</p>
        </div>
...
@if (session('import_errors'))
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
            <form x-data="ajaxForm" @submit.prevent="submit" action="{{ route('admin.dining_tables.import') }}" method="POST" enctype="multipart/form-data" class="lg:p-8 px-4 py-4 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <ul class="mt-2 list-disc list-inside text-sm text-gray-600 dark:text-gray-400 space-y-1">
                        @php
                            $model = new \App\Models\DiningTable();
                ...
                <div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <div class="flex gap-2">
                            <a href="{{ route('admin.dining_tables.downloadSample', ['format' => 'csv']) }}"
                            ...
                            <a href="{{ route('admin.dining_tables.downloadSample', ['format' => 'xlsx']) }}"
                    ...
                    <div class="overflow-x-auto">
                        <pre class="text-xs text-gray-600 dark:text-gray-400 font-mono">number,capacity,status,qr_code,show
Sample Value,123,Sample Value,Sample Value,1</pre>
                    </div>
                </div>

                <!-- Actions -->
                <div class="flex items-center justify-end gap-4 pt-8 border-t-2 border-gray-100 dark:border-gray-700 mt-8">
                    <a href="{{ route('admin.dining_tables.index') }}"
