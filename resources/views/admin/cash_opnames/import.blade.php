...
@section('title', 'Import Cash Opnames')

@section('content')
    <div class="space-y-6">
        <!-- Page Header -->
        <div>
            <h1 class="lg:text-2xl text-xl font-semibold text-gray-900 dark:text-white">Import Cash Opnames</h1>
            <p class="mt-1 lg:text-sm text-xs text-gray-600 dark:text-gray-400">Upload a CSV or Excel file (.csv, .xlsx, .xls) to import cashopnames</p>
        </div>
...
@if (session('import_errors'))
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
            <form x-data="ajaxForm" @submit.prevent="submit" action="{{ route('admin.cash_opnames.import') }}" method="POST" enctype="multipart/form-data" class="lg:p-8 px-4 py-4 space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <ul class="mt-2 list-disc list-inside text-sm text-gray-600 dark:text-gray-400 space-y-1">
                        @php
                            $model = new \App\Models\CashOpname();
                ...
                <div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <div class="flex gap-2">
                            <a href="{{ route('admin.cash_opnames.downloadSample', ['format' => 'csv']) }}"
                            ...
                            <a href="{{ route('admin.cash_opnames.downloadSample', ['format' => 'xlsx']) }}"
                    ...
                    <div class="overflow-x-auto">
                        <pre class="text-xs text-gray-600 dark:text-gray-400 font-mono">user_id,opname_date,expected_cash,actual_cash,expected_qris,actual_qris,expected_transfer,actual_transfer,difference,status,notes
Sample Value,2024-01-01,Sample Value,Sample Value,Sample Value,Sample Value,Sample Value,Sample Value,Sample Value,Sample Value,Sample Value</pre>
                    </div>
                </div>
